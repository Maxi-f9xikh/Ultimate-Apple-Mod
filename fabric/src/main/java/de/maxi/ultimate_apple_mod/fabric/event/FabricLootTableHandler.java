...
public class FabricLootTableHandler {
    ...
    public static void register() {
        LootTableEvents.MODIFY.register((resourceManager, lootManager, id, tableBuilder, source) -> {
            ...
            // evoker vanilla totem drop: handled in FabricMobDropHandler; pool suppression not supported in 1.20.1

            // ── Chest loot tables ──────────────────────────────────────────────
            else if (id.equals(rl("chests/simple_dungeon")))
                tableBuilder.pool(LootPool.lootPool().setRolls(ConstantValue.exactly(1))
